...
import { getLocation, getNearPost, getPost } from '../../api/postApi';
...
const ListPost = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if(result.content.length!==0){
                const address1 = result.content[0].address+', '+result.content[0].ward+', '+result.content[0].district+', '+result.content[0].city
                const allPost = await getLocation(address1,selectedCity,selectedDistrict,selectedWard,selectedRoomType);
                setListALLPost(allPost)
            }else{
                const allPost = await getLocation('',selectedCity,selectedDistrict,selectedWard,selectedRoomType);
                setListALLPost(allPost)
            }
    ...
    useEffect(() => {
        if(listALLPost.length !== 0){
            setCenter({ lat: listALLPost[0].latitude, lng: listALLPost[0].longitude });
        }
    }, [listALLPost]); 
                    ...
                    <LoadScript googleMapsApiKey={apiKey}>
                            ...
                            <InfoWindow
                               ...
                               <div className="info-window-content d-flex">
                                    ...
                                    <img src={`http://localhost:8080${selected.postImages[0].urlImage}`} alt="Hình ảnh bài đăng"/>
                                    <div>
